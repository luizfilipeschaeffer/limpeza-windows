...
function Read-LimpezaUpdateChoice {
    while ($true) {
        Write-Host ''
        Write-Host '   [S] Sim â€” baixar e reiniciar com a nova versao' -ForegroundColor White
        Write-Host '   [N] Nao â€” continuar a limpeza agora' -ForegroundColor White
        Write-Host ''
        ...
        if ($answer -in @('N', 'NAO', 'NÃƒO', 'NO')) { return $false }
...
'@ -ErrorAction Stop
        ...
        # Sem console ou ambiente restrito â€” segue em silencio
    }
}
...
function Get-ExistingScheduledTask {
    ...
    $startLine = $query | Where-Object {
        $_ -match '^(Hora de in[iÃ­]cio|Hora de inicio|Start Time)\s*:'
    } | Select-Object -First 1
...
function Register-LimpezaScheduledTask {
    ...
    # /TR direto no .exe (evita cmd/start /B â€” o schtasks interpreta /B como opcao invalida)
